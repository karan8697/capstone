		...
		<div class="col-sm-10 col-sm-offset-1  container-fluid" style="height: 200px;overflow-y:none ;">
			...
			<div class="col-sm-12 listening-question-box">
				...
				<div class="form-group ">
 					...
 					<div class="col-sm-10">
 						<input type="file" class="form-control" placeholder="Upload Video" name= "image" required>
 					</div>
				</div>
				...
				<?php
					...
					for($i=1;$i<=10;$i++)
					{
						...
						<div class="form-group ">
	   	 					...
	   	 					<div class="col-sm-10">
	   	 						<input type="text" class="form-control" placeholder="Enter answer <?php echo "$i"?>" name= "listening_answer_<?php echo "$i"?>" onkeyup="this.value = this.value.toUpperCase();" required>
	   	 					</div>
	  					</div>
						</div>
						<?php
...
<?php
    if(isset($_POST['submit']))
    {
        ...
        if($imageFileType != "mp4" && $imageFileType != "mpeg4" && $imageFileType != "flv"
        && $imageFileType != "mkv" ) 
        {
            ?>
            <script>
        ...
        {
        	$sql="insert into listening values ('$test_id','$answer_listening','$image')";
			if(mysqli_query($conn,$sql))
			{
				?>
				<script >
					alert("Listening Module Added");
					window.location.href="upload-reading.php?test_id=<?php echo "$test_id" ?>"
				</script>
				<?php
